...
import { forecloseLoanController, applyForecloseController, payforecloseLoanController, approveForecloseController } from "./foreclose.controller.js";

const forecloseRouter = Router();

const readLimiter = createRateLimiter({ windowMs: 60_000, max: 120, message: "Too many requests" });
const writeLimiter = createRateLimiter({ windowMs: 60_000, max: 60, message: "Too many requests" });
