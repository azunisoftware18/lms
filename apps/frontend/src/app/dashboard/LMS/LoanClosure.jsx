...
const RecoveryManagement = () => {
  ...
  const recoveryAgents = [
    { id: 1, name: 'Rajesh Kumar', activeCases: 45, recoveredAmount: 5678900, efficiency: 92, avatar: 'RK', team: 'Premium', joinDate: '2023-01-15' },
    { id: 2, name: 'Priya Singh', activeCases: 38, recoveredAmount: 4345678, efficiency: 88, avatar: 'PS', team: 'Corporate', joinDate: '2023-03-20' },
    { id: 3, name: 'Amit Patel', activeCases: 52, recoveredAmount: 6789012, efficiency: 85, avatar: 'AP', team: 'SME', joinDate: '2022-11-10' },
    { id: 4, name: 'Sunita Reddy', activeCases: 41, recoveredAmount: 3987654, efficiency: 79, avatar: 'SR', team: 'Retail', joinDate: '2023-06-05' },
    { id: 5, name: 'Vikram Mehta', activeCases: 47, recoveredAmount: 5234567, efficiency: 87, avatar: 'VM', team: 'Premium', joinDate: '2023-02-28' },
    { id: 6, name: 'Neha Gupta', activeCases: 35, recoveredAmount: 3456789, efficiency: 91, avatar: 'NG', team: 'Retail', joinDate: '2023-08-12' }
  ];
  ...
  const mockCallHistory = [
    { id: 1, date: '2024-03-15', agentName: 'Priya Singh', outcome: 'Promise to pay', remarks: 'Customer promised to pay by 20th March', duration: '8:32', sentiment: 'Positive' },
    { id: 2, date: '2024-03-10', agentName: 'Priya Singh', outcome: 'No response', remarks: 'Called 3 times, no answer', duration: '0:00', sentiment: 'Neutral' },
    { id: 3, date: '2024-03-05', agentName: 'Rajesh Kumar', outcome: 'Requested more time', remarks: 'Customer requested extension till 15th March', duration: '12:15', sentiment: 'Negative' }
  ];

  const mockLegalNotices = [
    { id: 1, sentDate: '2024-03-01', noticeType: 'Demand Notice', status: 'Delivered', response: 'Acknowledged' },
    { id: 2, sentDate: '2024-02-15', noticeType: 'Final Notice', status: 'Acknowledged', response: 'Requested settlement' }
  ];
  ...
  const handleConfirmAction = async () => {
    ...
    switch(action) {
      case 'assign':
        showToast(`Case ${data.loanNumber} assigned to ${data.agentName}`, 'success');
        break;
      case 'callLog':
  ...
  const getFilteredCases = () => {
    ...
    if (riskFilter !== 'all') {
      filtered = filtered.filter(c => c.riskCategory.toLowerCase() === riskFilter.toLowerCase());
    }
    
    if (statusFilter !== 'all') {
      filtered = filtered.filter(c => c.status.toLowerCase().includes(statusFilter.toLowerCase()));
    }
          ...
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-slate-500 text-sm">NBFC & Banking Recovery Operations</p>
              </div>
            </div>
            <div className="flex gap-3">
              ...
              <div className="relative group">
                ...
                <div className="absolute right-0 mt-2 w-48 bg-white rounded-lg shadow-lg border border-slate-100 hidden group-hover:block z-20">
                  <button onClick={() => handleExport('excel')} className="w-full text-left px-4 py-2 hover:bg-slate-50 text-slate-700">Excel Report</button>
                  <button onClick={() => handleExport('pdf')} className="w-full text-left px-4 py-2 hover:bg-slate-50 text-slate-700">PDF Report</button>
                  <button onClick={() => handleExport('csv')} className="w-full text-left px-4 py-2 hover:bg-slate-50 text-slate-700">CSV Format</button>
                </div>
              </div>
            </div>
          </div>
        ...
        {activeTab === 'dashboard' && (
          <div className="space-y-6">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 xl:grid-cols-6 gap-4">
              <StatCard title="Total Cases" value={recoveryMetrics.totalCases} change={12} icon={Users} color="bg-blue-500" subtext="+124 this month" />
              <StatCard title="Recovered Amount" value={`₹${(recoveryMetrics.recoveredAmount / 10000000).toFixed(2)}Cr`} change={8.5} icon={IndianRupee} color="bg-green-500" />
              <StatCard title="Recovery Rate" value={`${recoveryMetrics.recoveryPercentage}%`} change={5.2} icon={Target} color="bg-purple-500" />
              <StatCard title="Collection Efficiency" value={`${recoveryMetrics.collectionEfficiency}%`} change={3.1} icon={Activity} color="bg-orange-500" />
              <StatCard title="Active Agents" value={recoveryMetrics.totalAgents} change={0} icon={Briefcase} color="bg-cyan-500" />
              <StatCard title="Today's Calls" value={recoveryMetrics.todayCalls} change={-2.3} icon={PhoneCall} color="bg-pink-500" />
            </div>
              ...
              <div className="bg-white rounded-xl shadow-sm p-6 border border-slate-100">
                <div className="flex items-center justify-between mb-6">
                  <div className="flex items-center gap-2">
                    ...
                    <h2 className="text-lg font-semibold text-slate-800">Weekly Recovery Trend</h2>
                  </div>
                ...
                <ResponsiveContainer width="100%" height={300}>
                  <ComposedChart data={weeklyRecoveryData}>
                    ...
                    <Tooltip />
                    <Legend />
                    <Bar yAxisId="left" dataKey="amount" name="Recovered Amount (₹)" fill="#3B82F6" radius={[4, 4, 0, 0]} />
                    <Line yAxisId="right" type="monotone" dataKey="calls" name="Calls Made" stroke="#10B981" strokeWidth={2} />
                    <Line yAxisId="right" type="monotone" dataKey="promises" name="Promises" stroke="#F59E0B" strokeWidth={2} />
                  </ComposedChart>
                </ResponsiveContainer>
              </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
              ...
              <div className="bg-white rounded-xl shadow-sm p-6 border border-slate-100">
                <div className="flex items-center gap-2 mb-6">
                  ...
                  <h2 className="text-lg font-semibold text-slate-800">Monthly Recovery vs Target</h2>
                </div>
                <ResponsiveContainer width="100%" height={300}>
                  <BarChart data={monthlyRecoveryData}>
                    ...
                    <Tooltip />
                    <Legend />
                    <Bar dataKey="recovered" name="Recovered Amount" fill="#3B82F6" radius={[4, 4, 0, 0]} />
              ...
              <div className="bg-white rounded-xl shadow-sm p-6 border border-slate-100">
                <div className="flex items-center gap-2 mb-6">
                  ...
                  <h2 className="text-lg font-semibold text-slate-800">Top Agent Performance</h2>
                </div>
                <ResponsiveContainer width="100%" height={300}>
                  <BarChart data={agentPerformanceData} layout="vertical">
                    ...
                    <Tooltip />
                    <Legend />
                    <Bar dataKey="recovered" name="Amount Recovered (₹)" fill="#3B82F6" radius={[0, 4, 4, 0]} />
            ...
            <div className="bg-white rounded-xl shadow-sm border border-slate-100 overflow-hidden">
              <div className="p-4 border-b border-slate-200">
                <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                  <h3 className="text-lg font-semibold text-slate-800">High Priority Cases</h3>
                ...
                <table className="w-full">
                  <thead>
                    <tr className="bg-slate-50 border-b border-slate-200">
                      <th className="px-6 py-3 text-left text-xs font-medium text-slate-500 uppercase tracking-wider cursor-pointer hover:text-slate-700" onClick={() => handleSort('loanNumber')}>
                        Loan Number <ArrowUpDown className="w-3 h-3 inline ml-1" />
                      </th>
                      ...
                      <th className="px-6 py-3 text-left text-xs font-medium text-slate-500 uppercase tracking-wider">DPD</th>
                  ...
                  <tbody className="divide-y divide-slate-200">
                    {paginatedCases.slice(0, 5).map((case_) => (
                      <tr key={case_.id} className="hover:bg-slate-50 transition-colors">
                        <td className="px-6 py-4 text-sm font-medium text-slate-800">{case_.loanNumber}</td>
                        ...
                        <td className="px-6 py-4 text-sm font-semibold text-slate-800">₹{case_.outstandingAmount.toLocaleString()}</td>
                        <td className="px-6 py-4">
                          <span className={`text-sm font-medium ${case_.daysPastDue > 90 ? 'text-red-600' : case_.daysPastDue > 60 ? 'text-orange-600' : case_.daysPastDue > 30 ? 'text-yellow-600' : 'text-green-600'}`}>
                            {case_.daysPastDue} days
                          </span>
                        </td>
                        <td className="px-6 py-4">
                          <span className={`px-2 py-1 rounded-full text-xs font-medium ${getRiskColor(case_.riskCategory)}`}>
                            {case_.riskCategory}
                          </span>
                        </td>
                        ...
                        <td className="px-6 py-4">
                          <div className="flex items-center gap-2">
                            <button onClick={() => handleViewCase(case_.loanNumber)} className="p-1 hover:bg-slate-100 rounded-lg transition-colors" title="View Case">
                              <Eye className="w-4 h-4 text-slate-600" />
                            </button>
                            <button onClick={() => handleAddCallLog(case_.customerName)} className="p-1 hover:bg-slate-100 rounded-lg transition-colors" title="Add Call">
                ...
                >
                  ...
                  {mockRecoveryCases.map(case_ => (
                    <option key={case_.id} value={case_.loanNumber}>
                      {case_.loanNumber} - {case_.customerName} (₹{case_.outstandingAmount.toLocaleString()})
                    </option>
                  ))}
                </select>
              </div>
              ...
              <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                ...
                <div className="bg-white rounded-xl shadow-sm p-6 border border-slate-100 lg:col-span-1">
                  <div className="flex items-center gap-2 mb-4">
                    ...
                    <h2 className="text-lg font-semibold text-slate-800">Customer Profile</h2>
                  </div>
                  ...
                  <div className="space-y-4">
                    <div className="flex items-center gap-3 p-3 bg-slate-50 rounded-lg">
                      ...
                      <div>
                        <p className="font-semibold text-slate-800">{selectedCase.customerName}</p>
                    ...
                    <div className="space-y-3">
                      <div className="flex items-center gap-3">
                        ...
                        <span className="text-sm text-slate-600">{selectedCase.phone}</span>
                      </div>
                      <div className="flex items-center gap-3">
                        <Mail className="w-4 h-4 text-slate-400" />
                        <span className="text-sm text-slate-600">{selectedCase.email}</span>
                      </div>
                      <div className="flex items-center gap-3">
                        <MapPin className="w-4 h-4 text-slate-400" />
                        <span className="text-sm text-slate-600">{selectedCase.address}</span>
                      </div>
                    </div>
                  </div>
                </div>
                ...
                <div className="bg-white rounded-xl shadow-sm p-6 border border-slate-100 lg:col-span-1">
                  ...
                  <div className="space-y-3">
                    <div className="flex justify-between py-2 border-b border-slate-100">
                      ...
                      <span className="font-mono font-medium text-slate-800">{selectedCase.loanNumber}</span>
                    </div>
                    <div className="flex justify-between py-2 border-b border-slate-100">
                      <span className="text-slate-600">Outstanding Amount</span>
                      <span className="font-semibold text-red-600">₹{selectedCase.outstandingAmount.toLocaleString()}</span>
                    </div>
                    <div className="flex justify-between py-2 border-b border-slate-100">
                    ...
                    <div className="flex justify-between py-2 border-b border-slate-100">
                      ...
                      <span className={`font-semibold ${selectedCase.daysPastDue > 90 ? 'text-red-600' : selectedCase.daysPastDue > 60 ? 'text-orange-600' : 'text-yellow-600'}`}>
                ...
                <div className="bg-white rounded-xl shadow-sm p-6 border border-slate-100 lg:col-span-1">
                  ...
                  <div className="space-y-3">
                    <button onClick={() => handleAddCallLog(selectedCase.customerName)} className="w-full flex items-center justify-between p-3 bg-blue-50 rounded-lg hover:bg-blue-100 transition-colors">
                      <div className="flex items-center gap-3">
                        <PhoneCall className="w-5 h-5 text-blue-600" />
                        <span className="text-sm font-medium text-blue-700">Log Call</span>
                      </div>
                      <ArrowRight className="w-4 h-4 text-blue-600" />
                    </button>
                    
                    <button onClick={() => handleSendLegalNotice(selectedCase.customerName)} className="w-full flex items-center justify-between p-3 bg-orange-50 rounded-lg hover:bg-orange-100 transition-colors">
                      <div className="flex items-center gap-3">
                        <FileWarning className="w-5 h-5 text-orange-600" />
                        <span className="text-sm font-medium text-orange-700">Send Legal Notice</span>
                      </div>
                    ...
                    <button className="w-full flex items-center justify-between p-3 bg-green-50 rounded-lg hover:bg-green-100 transition-colors">
                      <div className="flex items-center gap-3">
                        ...
                        <span className="text-sm font-medium text-green-700">Schedule Follow-up</span>
                      </div>
                    ...
                    <button className="w-full flex items-center justify-between p-3 bg-slate-50 rounded-lg hover:bg-slate-100 transition-colors">
                      <div className="flex items-center gap-3">
                        ...
                        <span className="text-sm font-medium text-slate-700">Send Reminder</span>
                      </div>
                ...
                <div className="bg-white rounded-xl shadow-sm p-6 border border-slate-100 lg:col-span-3">
                  <div className="flex items-center gap-2 mb-4">
                    ...
                    <h2 className="text-lg font-semibold text-slate-800">Call History & Communication Log</h2>
                  </div>
                      ...
                      <div key={call.id} className="flex items-start gap-3 p-3 bg-slate-50 rounded-lg">
                        ...
                        <div className="flex-1">
                          <div className="flex items-center justify-between">
                            <p className="font-medium text-slate-800">{call.date} - {call.agentName}</p>
                          ...
                          <p className="text-sm text-slate-600 mt-1">{call.outcome}</p>
                          <p className="text-xs text-slate-500 mt-1">{call.remarks}</p>
                          <p className="text-xs text-slate-400 mt-1">Duration: {call.duration}</p>
                        </div>
                      </div>
                    ))}
                  </div>
            ...
            <div className="bg-white rounded-xl shadow-sm border border-slate-100 overflow-hidden">
              <div className="overflow-x-auto">
                <table className="w-full">
                  <thead>
                    <tr className="bg-slate-50 border-b border-slate-200">
                      <th className="px-6 py-3 text-left text-xs font-medium text-slate-500 uppercase tracking-wider">Loan Number</th>
                      <th className="px-6 py-3 text-left text-xs font-medium text-slate-500 uppercase tracking-wider">Customer</th>
                      <th className="px-6 py-3 text-left text-xs font-medium text-slate-500 uppercase tracking-wider">Outstanding</th>
                      <th className="px-6 py-3 text-left text-xs font-medium text-slate-500 uppercase tracking-wider">DPD</th>
                      <th className="px-6 py-3 text-left text-xs font-medium text-slate-500 uppercase tracking-wider">Risk</th>
                      <th className="px-6 py-3 text-left text-xs font-medium text-slate-500 uppercase tracking-wider">Current Agent</th>
                      <th className="px-6 py-3 text-left text-xs font-medium text-slate-500 uppercase tracking-wider">Reassign Agent</th>
                  ...
                  <tbody className="divide-y divide-slate-200">
                    {paginatedCases.map((case_) => (
                      <tr key={case_.id} className="hover:bg-slate-50 transition-colors">
                        <td className="px-6 py-4 text-sm font-mono font-medium text-slate-800">{case_.loanNumber}</td>
                        <td className="px-6 py-4 text-sm text-slate-600">{case_.customerName}</td>
                        <td className="px-6 py-4 text-sm font-semibold text-slate-800">₹{case_.outstandingAmount.toLocaleString()}</td>
                        <td className="px-6 py-4 text-sm font-medium">{case_.daysPastDue} days</td>
                        <td className="px-6 py-4">
                          <span className={`px-2 py-1 rounded-full text-xs font-medium ${getRiskColor(case_.riskCategory)}`}>
                            {case_.riskCategory}
                          </span>
                        ...
                        <td className="px-6 py-4">
                          <button
                            onClick={() => handleAssignAgent(case_.loanNumber, 'New Agent')}
        ...
        {activeTab === 'reports' && (
          <div className="space-y-6">
            ...
            <div className="bg-white rounded-xl shadow-sm p-4 border border-slate-100">
              <div className="flex flex-wrap gap-4 items-center">
                <div>
                  ...
                  <input type="date" className="ml-2 px-3 py-2 border border-slate-200 rounded-lg" />
                </div>
                <div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
              <StatCard title="Collection Rate" value="78.5%" change={5.2} icon={Target} color="bg-emerald-500" />
              <StatCard title="Roll Rate" value="12.3%" change={-2.1} icon={TrendingUp} color="bg-amber-500" />
              <StatCard title="Delinquency Rate" value="8.7%" change={-1.5} icon={AlertCircle} color="bg-red-500" />
              <StatCard title="Recovery Cost" value="₹2.3L" change={-3.2} icon={Banknote} color="bg-purple-500" />
            </div>

            {/* Agent Performance Table */}
            <div className="bg-white rounded-xl shadow-sm border border-slate-100 overflow-hidden">
              <div className="p-4 border-b border-slate-200">
                <h3 className="text-lg font-semibold text-slate-800">Agent Performance Dashboard</h3>
              </div>
              <div className="overflow-x-auto">
                <table className="w-full">
                  <thead>
                    <tr className="bg-slate-50 border-b border-slate-200">
                      ...
                      <th className="px-6 py-3 text-left text-xs font-medium text-slate-500 uppercase tracking-wider">Active Cases</th>
                      <th className="px-6 py-3 text-left text-xs font-medium text-slate-500 uppercase tracking-wider">Recovered Amount</th>
                      <th className="px-6 py-3 text-left text-xs font-medium text-slate-500 uppercase tracking-wider">Efficiency</th>
                      ...
                      <tr key={agent.id} className="hover:bg-slate-50">
                        <td className="px-6 py-4">
                          <div className="flex items-center gap-3">
                            ...
                            <div>
                              ...
                              <p className="text-xs text-slate-500">Joined {agent.joinDate}</p>
                            </div>
                          </div>
                        </td>
                        <td className="px-6 py-4">
                          <span className="px-2 py-1 bg-blue-100 text-blue-700 rounded-full text-xs font-medium">{agent.team}</span>
                        </td>
                        <td className="px-6 py-4 text-sm text-slate-600">{agent.activeCases}</td>
                        <td className="px-6 py-4 text-sm font-semibold text-green-600">₹{(agent.recoveredAmount / 100000).toFixed(1)}L</td>
                        <td className="px-6 py-4">
                          <div className="flex items-center gap-2">
                            <div className="flex-1 bg-slate-200 rounded-full h-2 w-24">
                              <div className="bg-green-500 rounded-full h-2" style={{ width: `${agent.efficiency}%` }}></div>
                            </div>
                        ...
                        <td className="px-6 py-4">
                          <div className="flex items-center gap-0.5">
                            {[...Array(5)].map((_, i) => (
                              <Star key={i} className={`w-4 h-4 ${i < Math.floor(agent.efficiency / 20) ? 'text-yellow-400 fill-yellow-400' : 'text-slate-300'}`} />
                            ))}
                          </div>
                        </td>
                      </tr>
