...
const applyAadhaarProfileToGuarantor = (profile, aadhaarNumber, index, setValue) => {
  if (!profile) return;
  const setField = (path, value) => {
    setValue(`guarantors.${index}.${path}`, value ?? "", {
      shouldDirty: true,
      shouldTouch: true,
    });
  };
...
const applyPanProfileToGuarantor = (profile, index, setValue) => {
  if (!profile) return;
  const setField = (path, value) => {
    setValue(`guarantors.${index}.${path}`, value ?? "", {
      shouldDirty: true,
...
export default function GuarantorSection({
  ...
  const handlePanFetch = async (index) => {
    ...
    try {
      ...
      setValue(`guarantors.${index}.panProvider`, payload);
      ...
      if (profile) {
        applyPanProfileToGuarantor(profile, index, setValue);
      }
  ...
  const handleVerifyOtp = async (index) => {
    ...
    try {
      ...
      try {
        ...
        if (profile) {
          applyAadhaarProfileToGuarantor(profile, aadhaar, index, setValue);
        }
      } catch (e) {
        console.error(`Failed to apply Aadhaar profile to guarantors.${index}:`, e);
      }
      setOtpVerifiedMap((s) => ({ ...s, [index]: true }));
      setShowOtpModalMap((s) => ({ ...s, [index]: false }));
