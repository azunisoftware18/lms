...
import { ChevronDown, X, Check, AlertCircle, Loader2, Search } from 'lucide-react';
      ...
      <div className="relative">
        ...
        >
          <div className="flex flex-wrap gap-1.5 flex-1">
            {/* Multi-Select Chips */}
            {isMulti && Array.isArray(value) && value.map(v => (
              <span key={v} className="flex items-center gap-1 bg-blue-100 text-blue-700 px-2 py-0.5 rounded text-xs font-medium">
                {options.find(o => o.value === v)?.label}
                <X size={14} className="cursor-pointer hover:text-blue-900" onClick={(e) => removeValue(e, v)} />
              </span>
            ))}
          ...
          <div className="flex items-center gap-2 ml-2">
            {isLoading ? <Loader2 size={16} className="animate-spin text-slate-400" /> : <ChevronDown size={18} className={`transition-transform ${isOpen ? 'rotate-180' : ''}`} />}
          </div>
        </div>
