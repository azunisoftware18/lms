...
export const useCoApplicants = (loanApplicationId, params = {}) => {
  ...
  const query = useQuery({
    queryKey: ["coApplicants", loanApplicationId, normalizedParams],
    queryFn: () =>
...
export const useCoApplicantById = (id) => {
  ...
  return useQuery({
    queryKey: ["coApplicant", id],
    queryFn: () => apiGet(`/co-applicant/detail/${id}`),
    enabled: !!id,
    onSuccess: () => {
      dispatch(clearError());
    },
...
export const useUpdateCoApplicant = () => {
  ...
  return useMutation(({ id, data }) => apiPut(`/co-applicant/${id}`, data), {
    onMutate: () => {
      dispatch(setLoading(true));
    },
    onSuccess: (data) => {
...
export const useReuploadCoApplicantDocument = () => {
  ...
  return useMutation(
    ({ coApplicantId, documentType, file }) => {
      const formData = new FormData();
