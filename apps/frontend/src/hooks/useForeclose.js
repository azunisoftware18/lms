...
export const useApplyForeclose = () => {
	...
	return useMutation({
		mutationFn: ({ loanNumber, data }) => apiPost(`/foreclose/loans/${loanNumber}/apply-foreclose`, data),
...
export const usePayForeclose = () => {
	...
	return useMutation({
		mutationFn: ({ loanNumber, data }) => apiPost(`/foreclose/loans/${loanNumber}/foreclose`, data),
		onMutate: () => dispatch(setLoading(true)),
		onSuccess: (res) => {
...
export const useApproveForeclose = () => {
	...
	return useMutation({
		mutationFn: ({ loanNumber, approve }) => apiPost(`/foreclose/loans/${loanNumber}/foreclose/approval`, { approve }),
		onMutate: () => dispatch(setLoading(true)),
		onSuccess: (res) => {
			queryClient.invalidateQueries(["foreclose"]);
			dispatch(setLoading(false));
			dispatch(clearError());
			showSuccess(approve ? "Foreclose approved" : "Foreclose rejected");
		},
		onError: (err) => {
